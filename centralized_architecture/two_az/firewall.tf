...
locals {
  firewall_endpoint_ids = [
    ...
    one([
      ...
      if state.attachment[0].subnet_id == aws_subnet.inspection_firewall[idx].id
    ])
  ]
}
...
resource "aws_cloudwatch_log_group" "firewall_flow" {
  name              = "/${var.project_name}/inspection-fw/flow"
...
resource "aws_cloudwatch_log_group" "firewall_alert" {
  name              = "/${var.project_name}/inspection-fw/alert"
  retention_in_days = var.log_retention_days
