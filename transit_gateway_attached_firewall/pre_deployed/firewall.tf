...
resource "aws_networkfirewall_firewall_policy" "main" {
  name = "inspection-firewall-policy-${var.project_name}"
...
#------------------------------------------------------------------------------
# Local value to extract TGW attachment ID from firewall status
#------------------------------------------------------------------------------
locals {
  firewall_tgw_attachment_id = one([
    for state in aws_networkfirewall_firewall.main.firewall_status[0].transit_gateway_attachment_sync_states :
    state.attachment_id
  ])
}
